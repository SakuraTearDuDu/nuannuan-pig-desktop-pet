...
const APP_DISPLAY_NAME = '闯祸鸡桌面宠物';
const PET_ID = 'chuanghuo-ji';
const BASE_ACTIONS = [
...
function validateAssets() {
  ...
  if (pet.id !== PET_ID || pet.spritesheetPath !== 'spritesheet.webp') {
    throw new Error('pet.json must describe the bundled Chuanghuo Ji pet and spritesheet.webp.');
  }
...
function createWindow() {
  ...
  mainWindow.loadFile(path.join(__dirname, 'renderer', 'chuanghuo-ji.html'));

  if (process.env.CHUANGHUO_JI_PET_SMOKE_TEST === '1') {
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('CHUANGHUO_JI_PET_SMOKE_LOADED');
    });
    mainWindow.once('ready-to-show', () => {
      console.log('CHUANGHUO_JI_PET_SMOKE_READY');
      setTimeout(() => {
        isQuitting = true;
        app.quit();
      }, 400);
