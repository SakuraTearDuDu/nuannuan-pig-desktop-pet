...
const COLUMNS = 24;
const IQ_DANCE_ROW_COUNT = 6;
const SCALES = [0.75, 1, 1.25, 1.5, 2];
const APP_DISPLAY_NAME = '跳舞鸡桌面宠物';
...
const TRAY_ICON_PATH = path.join(ASSETS_DIR, 'dance-chicken-tray.png');
...
function validateAssets() {
  if (!fs.existsSync(LONG_ACTIONS_PATH) || !fs.existsSync(LONG_ACTIONS_JSON_PATH)) {
    throw new Error('缺少 IQ 博士舞蹈专用动作资源。');
  }

  longActionsManifest = JSON.parse(fs.readFileSync(LONG_ACTIONS_JSON_PATH, 'utf8'));
  const size = parseWebpSize(LONG_ACTIONS_PATH);
  if (longActionsManifest.cellWidth !== CELL_WIDTH || longActionsManifest.cellHeight !== CELL_HEIGHT) {
    throw new Error('IQ 博士舞蹈动作必须使用 192x208 单元格。');
  }
  if (longActionsManifest.columns !== COLUMNS) {
    throw new Error('IQ 博士舞蹈图集必须使用 24 列。');
  }
  const iqDance = Array.isArray(longActionsManifest.actions)
    ? longActionsManifest.actions.find(action => action.id === 'iq-dance')
    : null;
  if (!iqDance) {
    throw new Error('IQ 博士舞蹈清单缺少 iq-dance 动作。');
  }
  const requiredRows = iqDance.row + (iqDance.rowCount || Math.ceil(iqDance.frames / COLUMNS));
  if (iqDance.rowCount !== IQ_DANCE_ROW_COUNT || iqDance.frames !== COLUMNS * IQ_DANCE_ROW_COUNT) {
    throw new Error('IQ 博士舞蹈动作必须是 24 列 x 6 行。');
  }
  if (size.width !== CELL_WIDTH * COLUMNS || size.height < CELL_HEIGHT * requiredRows) {
    throw new Error('IQ 博士舞蹈图集尺寸和清单不一致。');
  }
}
...
function createWindow() {
  ...
  mainWindow.loadFile(path.join(__dirname, 'renderer', 'iq-dance.html'));

  if (process.env.SIYANJI_IQ_DANCE_SMOKE_TEST === '1') {
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('SIYANJI_IQ_DANCE_SMOKE_LOADED');
    });
    mainWindow.once('ready-to-show', () => {
      console.log('SIYANJI_IQ_DANCE_SMOKE_READY');
      setTimeout(() => {
        isQuitting = true;
        app.quit();
      }, 400);
...
ipcMain.handle('pet:move-drag', () => {
  ...
  const size = windowSize();
  mainWindow.setBounds({
    x: Math.round(dragState.bounds.x + cursor.x - dragState.screenX),
    y: Math.round(dragState.bounds.y + cursor.y - dragState.screenY),
    ...size
  });
});
