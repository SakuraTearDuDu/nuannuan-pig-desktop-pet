...
const APP_DISPLAY_NAME = 'mini酋长鸡桌面宠物';
const PET_ID = 'mini-chieftain-chicken';
const BASE_ACTIONS = [
...
const ASSETS_DIR = path.join(ROOT_DIR, 'assets', PET_ID);
...
function validateAssets() {
  ...
  if (pet.id !== PET_ID || pet.spritesheetPath !== 'spritesheet.webp') {
    throw new Error('pet.json must describe the bundled mini chieftain chicken pet and spritesheet.webp.');
  }
...
function createWindow() {
  ...
  mainWindow.loadFile(path.join(__dirname, 'renderer', 'mini-chieftain-chicken.html'));

  if (process.env.MINI_CHIEFTAIN_CHICKEN_PET_SMOKE_TEST === '1') {
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('MINI_CHIEFTAIN_CHICKEN_PET_SMOKE_LOADED');
    });
    mainWindow.once('ready-to-show', () => {
      console.log('MINI_CHIEFTAIN_CHICKEN_PET_SMOKE_READY');
      setTimeout(() => {
        isQuitting = true;
        app.quit();
      }, 400);
