...
const petId = 'gui-fei-ji-student-uniform-pixel';
...
function main() {
  ...
  if (pet.displayName !== '贵妃鸡（学生服）') {
    throw new Error(`Expected displayName 贵妃鸡（学生服）; found ${pet.displayName}`);
  }
...
try {
  ...
  console.error(`Gui Fei Ji asset validation failed: ${error.message}`);
  process.exit(1);
}
