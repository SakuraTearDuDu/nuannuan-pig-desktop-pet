...
const assetsDir = path.join(root, 'assets', 'chuanghuo-ji');
...
const iconPath = path.join(root, 'build', 'chuanghuo-ji-icon.ico');
const macIconPath = path.join(root, 'build', 'chuanghuo-ji-icon.icns');
...
function main() {
  ...
  if (pet.id !== 'chuanghuo-ji') {
    throw new Error(`Expected pet id chuanghuo-ji; found ${pet.id}`);
  }
  if (pet.displayName !== '闯祸鸡') {
    throw new Error(`Expected displayName 闯祸鸡; found ${pet.displayName}`);
  }
...
try {
  ...
  console.error(`Chuanghuo Ji asset validation failed: ${error.message}`);
  process.exit(1);
}
