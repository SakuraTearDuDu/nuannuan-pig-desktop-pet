...
const assetsDir = path.join(root, 'assets', 'mini-chieftain-chicken');
...
const iconPath = path.join(root, 'build', 'mini-chieftain-chicken-icon.ico');
const macIconPath = path.join(root, 'build', 'mini-chieftain-chicken-icon.icns');
...
function main() {
  ...
  if (pet.id !== 'mini-chieftain-chicken') {
    throw new Error(`Expected pet id mini-chieftain-chicken; found ${pet.id}`);
  }
  if (pet.displayName !== 'mini酋长鸡') {
    throw new Error(`Expected displayName mini酋长鸡; found ${pet.displayName}`);
  }
...
try {
  ...
  console.error(`Mini chieftain chicken asset validation failed: ${error.message}`);
  process.exit(1);
}
